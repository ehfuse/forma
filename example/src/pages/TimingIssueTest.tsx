...
function CustomerSeqDisplay() {
    ...
    console.log("🔵 dotNotationListeners.size:", (_store as any).dotNotationListeners.size);
    
    return (
        <div style={{ padding: '10px', background: '#e3f2fd', margin: '10px 0' }}>
            <h4>CustomerSeqDisplay (구독: customer.seq)</h4>
            <p>customer.seq = <strong>{customer_seq || '(비어있음)'}</strong></p>
        </div>
    );
}
...
function CustomerNameDisplay() {
    ...
    return (
        <div style={{ padding: '10px', background: '#f3e5f5', margin: '10px 0' }}>
            <h4>CustomerNameDisplay (구독: customer.name)</h4>
            <p>customer.name = <strong>{customer_name || '(비어있음)'}</strong></p>
        </div>
    );
}
...
// 컨트롤러 컴포넌트
function Controller() {
    const { setValues, getValue, reset, _store } = useFormaState<FormData>(defaultValues, { 
        _externalStore: getOrCreateStore()
    });
    
    const handleSetValuesImmediate = () => {
        console.log("🔴 [즉시 설정] setValues 호출 전 dotNotationListeners.size:", 
            (_store as any).dotNotationListeners.size);
        
        setValues({
            customer: { seq: '2101', name: '홍길동' }
        });
        
        console.log("🔴 [즉시 설정] setValues 호출 후:");
        console.log("   - getValue('customer.seq'):", getValue('customer.seq'));
        console.log("   - getValue('customer.name'):", getValue('customer.name'));
    };
    ...
    const handleSetValuesDelayed = () => {
        ...
        setTimeout(() => {
            console.log("🟡 [지연 설정] setValues 호출 전 dotNotationListeners.size:", 
                (_store as any).dotNotationListeners.size);
            
            setValues({
                customer: { seq: '3202', name: '김철수' }
            });
    ...
    return (
        <div style={{ padding: '10px', background: '#fff3e0', margin: '10px 0' }}>
            <h4>Controller</h4>
            <button onClick={handleSetValuesImmediate} style={{ margin: '5px' }}>
                즉시 setValues (2101, 홍길동)
            </button>
            <button onClick={handleSetValuesDelayed} style={{ margin: '5px' }}>
                1초 후 setValues (3202, 김철수)
            </button>
            ...
            <div style={{ marginTop: '10px', fontSize: '12px', color: '#666' }}>
                <p>현재 값 (getValue):</p>
                <p>- customer.seq: {getValue('customer.seq') || '(비어있음)'}</p>
                <p>- customer.name: {getValue('customer.name') || '(비어있음)'}</p>
            </div>
        </div>
    );
}
...
export default function TimingIssueTest() {
    ...
    const handleOpenModalThenSetValues = () => {
        console.log("\n" + "=".repeat(80));
        console.log("🚀 시나리오: 모달 열기 → 즉시 setValues (타이밍 이슈 재현)");
        ...
        setTimeout(() => {
            ...
            console.log("📊 setValues 호출 시점의 dotNotationListeners.size:", 
                (store as any).dotNotationListeners.size);
            
            store.setValues({
                customer: { seq: '1001', name: '이영희' }
            });
    ...
    return (
        <div style={{ padding: '20px' }}>
            ...
            <p>
                <strong>테스트 목적:</strong> setValues() 호출 타이밍과 useValue() 구독 타이밍 확인
            </p>
            
            <div style={{ background: '#f5f5f5', padding: '15px', margin: '20px 0', borderRadius: '8px' }}>
                <h3>📌 문제 설명</h3>
                <p>
                    기존 <code>useState + useEffect</code> 방식에서는:
                </p>
                <ol>
                    <li>컴포넌트가 렌더링되면 <code>useState</code>가 즉시 실행</li>
                    <li><code>useEffect</code>는 렌더링 <strong>완료 후</strong>에 실행</li>
                    <li>그 사이에 <code>setValues()</code>가 호출되면 구독자가 없어서 알림 못 받음</li>
                    <li>구독 등록 후에도 이미 값은 변경되어 있어서 업데이트 안 됨</li>
                </ol>
                <p style={{ color: '#1976d2', fontWeight: 'bold', marginTop: '10px' }}>
                    ✅ <code>useSyncExternalStore</code>는 구독을 동기적으로 등록하여 이 문제 해결!
                </p>
            </div>
            
            <div style={{ background: '#e8f5e9', padding: '15px', margin: '20px 0', borderRadius: '8px' }}>
                <h3>🧪 테스트 시나리오</h3>
                <button 
            ...
            {showModal && (
                <div style={{ 
                    ...
                    <h3>📋 Modal (Form Components)</h3>
                    <p style={{ color: '#666', fontSize: '14px' }}>
                        아래 컴포넌트들은 각각 customer.seq와 customer.name을 구독합니다.
                    </p>
                    <CustomerSeqDisplay />
                    <CustomerNameDisplay />
                    <Controller />
            ...
            <div style={{ background: '#fff3e0', padding: '15px', margin: '20px 0', borderRadius: '8px' }}>
                <h3>📝 검증 방법</h3>
                <ol>
                    <li>
                        <strong>"모달 열고 즉시 setValues"</strong> 버튼 클릭
                        <ul>
                            <li>콘솔에서 <code>dotNotationListeners.size</code> 확인</li>
                            <li>구독자 등록 전/후 시점 확인</li>
                            <li><strong>화면에 값이 표시되는지 확인</strong> (핵심!)</li>
                        </ul>
                    </li>
                    <li>
                ...
                <ul style={{ lineHeight: '1.8' }}>
                    <li>모든 시나리오에서 <code>useValue</code>가 업데이트되어야 함</li>
                    <li>타이밍과 상관없이 구독이 동기적으로 등록되어야 함</li>
                    <li><code>setValues</code> 호출 시 즉시 모든 구독자가 알림받아야 함</li>
                    <li>콘솔에 customer.seq와 customer.name 값이 정확히 표시되어야 함</li>
                </ul>
            </div>
        </div>
    );

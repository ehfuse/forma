...
interface UseGlobalFormProps<T> {
    ...
    /** Custom actions (computed getters and handlers) - can be object or array */
    actions?: Actions<T> | Actions<T>[];
    /** Field change watchers - watch specific fields using dot notation */
    watch?: WatchConfig<T>;
    /** Custom actions (computed getters and handlers) - can be object or array */
    actions?: Actions<T> | Actions<T>[];
}
...
|                | `watch`         | `WatchConfig<T>`                                             | Field change watchers - supports dot notation (optional).                     |
...
Use the `watch` option to monitor specific field changes and react automatically.
...
const state = useGlobalFormaState({
    ...
    watch: {
        // Automatically execute when logined field changes
        logined: (context, value, prevValue) => {
            console.log(`Login state changed: ${prevValue} -> ${value}`);
        ...
        // Watch nested fields using dot notation
        "user.name": (context, value, prevValue) => {
            console.log(`User name changed: ${prevValue} -> ${value}`);
        },
...
interface ActionContext<T> {
    ...
    reset: () => void;
    values: T;
    actions: any; // Defined custom actions
}
```
...
const appState = useGlobalFormaState({
    ...
    watch: {
        logined: async (context, value) => {
            if (value) {
                // Start data synchronization on login
                const intervalId = setInterval(async () => {
                    await syncUserData();
                }, 5000);
                context.setValue("syncInterval", intervalId);
            } else {
                // Stop synchronization on logout
